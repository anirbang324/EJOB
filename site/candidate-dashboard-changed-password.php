...
if(isset($_POST['submit']))
			...
			if($error=='')
			{
			$sql_q=mysqli_query($connt,"SELECT * FROM tbl_candidate WHERE password='".mysqli_real_escape_string($connt,trim($_POST['password']))."' AND id=".$_SESSION['candidate_id']." "  );
			
			if(mysqli_num_rows($sql_q)>0)
			{
				$sql="UPDATE tbl_candidate SET password='" .mysqli_real_escape_string($connt,trim($_POST['conframpassword']))."' WHERE id=".$_SESSION['candidate_id']." ";
