...
if((isset($_POST['submit'])) AND ($_POST['submit']=='Sign In'))	{
	...
	$sql="select * from tbl_candidate where email = '".$_POST["email"]."'AND password = '".$_POST["password"]."'";
	...
	if(mysqli_num_rows($query)>0)
		{
				...
				if(isset($_POST['orderid'])){
					// CHECK USER ALREADY APPLYED THIS JOB OR NOT
					$sql_job_aply = "SELECT * FROM tbl_candidate_applied_for_job WHERE candidate_id = '".$data['id']."'  AND job_id = '".$_POST['orderid']."'";
					$query_job_aply=mysqli_query($connt,$sql_job_aply);
					if(mysqli_num_rows($query_job_aply)>0){	
						header("Location:job-detail.php?jobid=".$_POST['orderid']."&op=applyalready");
					}else{
						header("Location:job-detail.php?jobid=".$_POST['orderid']."&op=applysuccess");
					}
				}else{
                        ...
                        <div class="careerfy-column-12 careerfy-typo-wrap">
                            <figure class="careerfy-jobdetail-list">
                                <figcaption>
									...
									<form action="<?php echo $_SERVER["PHP_SELF"] ?>" name="candidate_login" id="candidate_login" method="post">
										...
										<div class="careerfy-user-form careerfy-user-form-coltwo">
											<ul>
												...
												<li >
													<label></label>
													<a href="candidate-signup.php">Dont have an account? Signup Now </a>
												</li>												
